<template>
  <div class="playlist-page">
    ...
    <div
      class="playlist-atmosphere"
      :class="{ active: library.isPlaying }"
...
const playlist = computed(() =>
  library.playlists.find((p) => p.id === route.params.playlistId),
);
...
watch(
  ...
  (pl) => {
    if (pl) {
      library.recordPlaylistPlayed(pl, playlistSongs.value.length);
    }
  },
  { immediate: true },
);
...
function openEditModal() {
  ...
  editPlaylistModal.value.open(playlist.value);
}
...
const isSortablePlaylist = computed(
  () => playlist.value && playlist.value.id !== "all",
);
