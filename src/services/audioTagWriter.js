...
async function renameFile(fileHandle, directoryHandle, currentName, title, extension) {
  ...
  const requestedName = `${safeFileName(title)}${cleanExt}`;
  if (requestedName === currentName) return { fileHandle, name: currentName };
  ...
  while (true) {
    try {
      ...
      if (await nextHandle.isSameEntry?.(fileHandle)) {
        return { fileHandle, name: currentName };
      }
      nextName = `${safeFileName(title)} (${suffix})${cleanExt}`;
      suffix += 1;
    } catch {
      try {
